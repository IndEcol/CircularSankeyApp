...
{
    public partial class frmCircularSankey : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string d_year = Request.QueryString["year"];
            string d_country = Request.QueryString["country"];

            if (D != null)
            {
                objDataHandling.setNodeAndFlowValues(input_node_data, input_flow_data, d_year, d_country);
            }

            if ((user_name == null || user_name == "") && D == null)
            {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            objDataHandling.unit_name = Convert.ToString(unit_name.Text);
            objDataHandling.flow_width= Convert.ToDouble(flow_width.Value);
            objDataHandling.node_height= Convert.ToDouble(node_height.Value);
        ...
        private ArrayList getControllersID(ArrayList controllersID)
        {
            ...
            controllersID.Add("flow_width");
            controllersID.Add("node_height");
        ...
        private void setControllersData(ArrayList controllers_data)
        {
            ...
            curvature.Attributes["value"] = Convert.ToString(controllers_data[9]);
            canvas_width.Text = Convert.ToString(controllers_data[11]);
            canvas_height.Text = Convert.ToString(controllers_data[10]);
            default_flow_opacity.Attributes["value"] = Convert.ToString(controllers_data[13]);
            default_node_opacity.Attributes["value"] = Convert.ToString(controllers_data[14]);
            node_border.SelectedIndex = Convert.ToInt16(controllers_data[8]);
            //show_labels.Checked = Convert.ToBoolean(controllers_data[18]);
            //font_face.SelectedIndex = Convert.ToIndex(controllers_data[16])
            font_size.Text = Convert.ToString(controllers_data[17]);
            font_color.Text = Convert.ToString(controllers_data[15]);
            unit_name.Text = Convert.ToString(controllers_data[19]);
            flow_width.Attributes["value"] = Convert.ToString(controllers_data[20]);
            node_height.Attributes["value"] = Convert.ToString(controllers_data[21]);
        }
        ...
        protected void btnNodeFile_Click(object sender, EventArgs e)
        {
            //if File is not selected then return  
            if (Request.Files["fileNode"].ContentLength <= 0)
            {
                return;
            }

            //Get the file extension  
            string fileExtension = Path.GetExtension(Request.Files["fileNode"].FileName);
            ...
            string fileLocation = Server.MapPath("\\") + Request.Files["fileNode"].FileName;
            ...
            Request.Files["fileNode"].SaveAs(fileLocation);
            ...
            foreach (Microsoft.Office.Interop.Excel.Worksheet sheet in xlWorkBook.Worksheets)
            {
                ...
                if (sheetcount==0)
                    ...
                    input_node_data.InnerText = data;
                sheetcount++;
